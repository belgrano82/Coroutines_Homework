        ...
        launch {
            try {
                ...
                val postAuthors = posts.map { post ->
                    async {
                        val author = getPostsAuthor(client, post.authorId)
                        Pair(post, author)
                    }
                }.awaitAll()


                val postsWithComments = postAuthors.map { (post, author) ->
                    async {
                        val postAuthor = getPostsAuthor(client, post.authorId)
                        PostWithComments(
                            post.copy(authorId = postAuthor.name),
                            getComments(client, post.id).map { comment ->
                                val commentAuthor = getPostsAuthor(client, comment.authorId)
                                comment.copy(authorId = commentAuthor.name)
                            }
                        )
                    }
                }.awaitAll()
...
suspend fun getPostsAuthor(client: OkHttpClient, id: String): Author =
